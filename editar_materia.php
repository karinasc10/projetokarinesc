...
if (isset($_POST['editar_materia'])) {
    ...
    $materia_id = $_GET['id']; // ID da nota a ser editada
   ...
   // $conteudo = $conexao->real_escape_string($conteudo);
    ...
    $sql = "UPDATE `materias` SET nome = '$nome', descricao = '$descricao', conteudo = '$conteudo', ordem = '$ordem' WHERE id = $materia_id";
                    ...
                    <div class="col-lg-10 text-center pt-5 mt-5">
                        ...
                        <form method="post" action="cadastrar_materia.php" class="">                                       
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nome" required value="<?= $notas['nome'] ?>">>
                            </div>
                            <div class="form-group">
                                <label for="nota">descrição:</label>
                                <input type="text" class="form-control" name="descricao" value="<?= $notas['descricao'] ?>">>
                            </div>
                            <div class="form-group">
                                <label for="nota">Orderm do menu:</label>
                                <input type="number" class="form-control" name="ordem"  value="<?= $notas['ordem'] ?>">>
                            </div>
                            <div class="form-group">
                                <label for="observacoes">Conteudo:</label>
                                <textarea class="form-control" name="conteudo" id="editor" rows="20" cols="50" style="height: 800px;" ><?= $notas['conteudo'] ?></textarea>
      <script>
                        ClassicEditor
...
editor.style.width = '100%';
                            ...
                            <button type="submit" class="btn btn-primary m-5">Editar máteria</button>
                        </form>
